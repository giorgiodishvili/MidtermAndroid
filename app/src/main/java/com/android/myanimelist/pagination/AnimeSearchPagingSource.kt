...
class AnimeSearchPagingSource(private val searchWord: String) : PagingSource<Int, AnimeSearchSubEntity >() {


    override fun getRefreshKey(state: PagingState<Int, AnimeSearchSubEntity>): Int? {
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, AnimeSearchSubEntity> {
        val position = params.key ?: STARTING_PAGE_INDEX
        return try {
            val response = RetrofitService.RETROFIT.searchAnime(searchWord, position)

            val data: List<AnimeSearchSubEntity> = response.body()!!.results
